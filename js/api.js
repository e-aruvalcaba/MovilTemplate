...
function SCOInitialize() {
	...
	if (!g_bInitDone) {
		...
		if (!APIOK()) {
			...
			estatu = g_objAPI.LMSGetValue("cmi.core.lesson_status");
			err = g_objAPI.LMSInitialize("");
			if (err == "true") {
				g_bSCOBrowse = (g_objAPI.LMSGetValue("cmi.core.lesson_mode") == "browse");
				if (!g_bSCOBrowse) {
					if (estatu != "incomplete" && estatu != "completed" && estatu != "passed" && estatu != "failed") {
						if (g_bInitializeOnLoad) {
							//alert("SCOInitialize CAMBIAR A INCOMPLETO  estatu= "+estatu);
	...
	if (APIOK()) {
		estatu = g_objAPI.LMSGetValue("cmi.core.lesson_status");
		if (g_bInitializeOnLoad && estatu != "incomplete" && estatu != "completed" && estatu != "passed" && estatu != "failed") {
			//alert("SCOInitialize 2 CAMBIAR A INCOMPLETO   estatu= "+estatu);
...
function SCOSetValue(nam, val) {
	...
	// se agreg� esta linea para asegurar la comunicaci�n 21-Abr
	SCOCommit();
	return err
}
...
// -------------------------   ACTUALIZACION 2014 PARA FUNCIONAR CON IE11 Y EXTERNAL INTERFASE --------------------------
var core_lesson_tries = 0;
var nswfObjv2 = null;
var g_bIsInternetExplorer;
var navDectected = new Browser();
if (navDectected.code == "ie") {
	g_bIsInternetExplorer = true;
} else {
	g_bIsInternetExplorer = false;
}
...
function RecabaDatos() {
	//alert("RecabaDatos");
	//nswfObjv2=swfobject.getObjectById("nswf");
	//alert("RecabaDatos nswfObjv2= "+nswfObjv2);
	var sdata;
	var pvez;
	var estatus = SCOGetValue("cmi.core.lesson_status");
	var sdata = '';
	core_lesson_tries++;
	pvez = SCOGetValue("cmi.suspend_data");
	if (pvez == "" || String(pvez) == "null" || String(pvez) == "false" || String(pvez) == "unknow" || String(pvez) == "undefined") {
		pvez = "YES";
		// console.log("Primera Vez");
		if (estatus != "incomplete" && estatus != "completed" && estatus != "passed" && estatus != "failed") {
			//alert("RecabaDatos CAMBIAR A INCOMPLETO   estatus = "+estatus);
			//SCOSetValue("cmi.core.lesson_status", "incomplete");
		}
	} else {
		pvez = "NO";
	...
	//	EnviarAobjSwf("First_Time",pvez);
	//	EnviarAobjSwf("student_name",SCOGetValue("cmi.core.student_name"));
	//	EnviarAobjSwf("ultimo_tema",SCOGetValue("cmi.core.lesson_location"));
	//	EnviarAobjSwf("score",SCOGetValue("cmi.core.score.raw"));
	sdata = SCOGetValue("cmi.suspend_data");
	if (sdata.substr(0, 5) == "temp=") {
		sdata = sdata.substr(5);
	}
	//	EnviarAobjSwf("core_lesson",sdata);
	// Con esta funcion se recupera del lms los datos deseados calif, nombre alumno, primera vez, suspend data etc y se envia a starJS
	//EnviarAstartJS(pvez, SCOGetValue("cmi.core.student_name"), SCOGetValue("cmi.core.score.raw"), sdata);
}

function EnviarAobjSwf(p1, p2) {
	//alert("EnviarAobjSwf  p1= "+p1+"  p2= "+p2);
	nswfObjv2.enviar_a_flash(p1, p2);
}

function new_nswf_DoFSCommand(comando, argumen) {
	//nswfObjv2=swfobject.getObjectById("nswf");
	//alert("new_nswf_DoFSCommand  nswfObjv2= "+nswfObjv2+"  comando= "+comando+"     argumen= "+ argumen);
	if (comando == "getParam") {
		RecabaDatos();
		...
		nswf_DoFSCommand(comando, argumen);
	}
}
// -------------------------   ACTUALIZACION 2014 PARA FUNCIONAR CON IE11 Y EXTERNAL INTERFASE --------------------------
...
function nswf_DoFSCommand(command, args) {
	...
	// 	nswfObj=nswfObjv2;
	// }else{
	// 	nswfObj = g_bIsInternetExplorer ? nswf : document.nswf;
	// }

	// no funciona si no hay disponible una API de SCORM
	//alert("nswf_DoFSCommand  nswfObj= "+nswfObj);
...
function monitor(cargadorObj) {
	var sdata;
	var pvez;
	var sdata = '';
	var estatus = SCOGetValue("cmi.core.lesson_status");
	core_lesson_tries++;
	pvez = SCOGetValue("cmi.core.lesson_status");
	if (pvez == "not attempted") {
		pvez = "YES";
		if (estatus != "incomplete" && estatus != "completed" && estatus != "passed" && estatus != "failed") {
			//SCOSetValue("cmi.core.lesson_status", "incomplete");
		}
	...
	cargadorObj.SetVariable("First_Time", pvez);
	cargadorObj.SetVariable("student_name", SCOGetValue("cmi.core.student_name"));
	cargadorObj.SetVariable("ultimo_tema", SCOGetValue("cmi.core.lesson_location"));
	cargadorObj.SetVariable("score", SCOGetValue("cmi.core.score.raw"));
	sdata = SCOGetValue("cmi.suspend_data");
	if (sdata.substr(0, 5) == "temp=") sdata = sdata.substr(5);
	cargadorObj.SetVariable("core_lesson", sdata);
...
function ComunicacionLMS(LMSpeticion, valor) {
	//alert("Comunicacion LMS y Peticion: "+LMSpeticion);
	var retorno = "false";
	var Peticion = LMSpeticion;
	// if(isArray(LMSpeticion)){
	// 	Peticion=LMSpeticion[0];
	// }else{
	// 	Peticion=LMSpeticion;
	// }
	switch (Peticion) {
		case "leer":
			retorno = SCOGetValue("cmi.suspend_data");
			SCOCommit();
			//alert("SUSPEND_DATA LEIDO y retorno "+retorno);
			break;
		case "guardar":
			retorno = SCOSetValue("cmi.suspend_data", valor);
			SCOCommit();
			// console.log("SUSPEND_DATA ACTUALIZADO.");
			break;
		case "completado":
			//mara como completado y actualiza el score
			retorno = SCOSetStatusCompleted();
			SCOSetValue("cmi.core.score.raw", valor);
			SCOCommit();
			break;
		case "estado":
			retorno = SCOGetValue("cmi.core.lesson_status");
			SCOCommit();
			break;
	}
...
function getStudentName() {
	return SCOGetValue("cmi.core.student_name");
}
...
function Browser() {
	...
	this.init = function() { //operative system, is an auxiliary var, for special-cases
		...
		var navs = [{
			...
			// { name:'IE11', fullName:'Internet Explorer', pre:'Trident/' } //add new broswers
			// { name:'', fullName:'', pre:'' }
			{
				name: 'Firefox',
		...
		if (agent.indexOf("Trident/") != -1 && agent.indexOf("rv:") != -1) {
			var nVer = agent.split("rv:").pop().split(")").shift();
			var nFVer = nVer;
			var nSVer = nVer.split(".").shift();
			this.fullVersion = parseFloat(nFVer);
			this.version = parseInt(nSVer);
			this.name = "iexplorer";
			this.fullName = "Internet Explorer";
		}
		//set version
